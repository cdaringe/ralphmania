...
`ensureProgressFile` accepts an injected `ProgressFileDeps` (defaulting to Deno)
instead of calling `Deno.*` directly:
...
1. **10 module purity tests** — read each pure-domain source file, strip
   ...
   - `src/command.ts`, `src/exit.ts`, `src/parsers/progress-rows.ts`,
     `src/set-fns.ts`
