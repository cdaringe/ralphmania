...
export const ensureProgressFile = async (log: Logger): Promise<void> => {
  const exists = await Deno.stat(PROGRESS_FILE).then(() => true, () => false);
  if (exists) return;
  ...
  log({
    tags: ["info", "progress"],
    message: `Created ${PROGRESS_FILE} with ${count} scenario rows — fill it in as you implement scenarios.`,
  });
};
