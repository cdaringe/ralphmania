...
const EXCLUDED_FILES: ReadonlyMap<string, string> = new Map([
  ...
  ["src/cli.ts", "CLI entry point with process-level orchestration and interactive prompts"],
  // Runner contains subprocess-spawning code that requires real agent binaries
  ["src/runner.ts", "Subprocess orchestration requiring real agent binaries (claude/codex)"],
  ...
  ["src/validation.ts", "Spawns bash subprocesses for validation script execution"],
  ...
  ["src/colors.ts", "Module-level Deno.stdout.isTerminal() prevents branch coverage in CI"],
  ...
  ["src/model.ts", "V8 coverage tool misattributes multi-line expressions; defensive catch branches unreachable"],
  // Orchestrator private helpers (prefixLog, readProgressContent, runWorker) only run with real worktrees
  ["src/orchestrator.ts", "Private helpers require real git worktrees and subprocess execution"],
  // Progress has defensive .catch branches for file I/O that cannot be triggered in tests
  ["src/progress.ts", "Defensive .catch branches for file I/O unreachable in test environment"],
]);
...
const filePattern = /\|\s*\S+\/src\/([^\s|]+)\s*\|\s*([\d.]+)\s*\|\s*([\d.]+)\s*\|/;
...
console.log("Coverage check passed: all files at 100% (or excluded with justification).");
